...
from bot import DRIVES_IDS, INDEX_URLS, DRIVES_NAMES, user_data
...
class gdSearch(GoogleDriveHelper):
    def __init__(self, stopDup=False, noMulti=False, is_recursive=True, itemType=""):
        super().__init__()
    ...
    def _drive_query(self, dirId, fileName, is_recursive):
        try:
            if is_recursive:
                if self._stopDup:
